...
class Admin_TinyPNG {
	...
	echo '<table id="myTable" class="bordered full_width tablesorter tp-tablesorter">
			<thead> 
			<tr> 
				<th>Select</th> 
				<th>Image Name</th> 
		...
		foreach ($data as $file) {
			...
			echo'	<td>'.$this->human_filesize($file["size"]).'</td>'; 
	...
	function compress_one(){
		...
		try {
			...
			$page->ajaxReplace[] = array('CompressOneRespond', 'arg', $e);
			return ;	
		}
	...
	gpFiles::SaveData($cache_file,'cache_data',$data);	
	$arg_value = true;	
	$page->ajaxReplace[] = array('CompressOneRespond', 'arg', $arg_value);
	return ;	
	}
	...
	//	$img_urls = explode( ',', $img_urls ) ;
		...
		foreach ($img_urls as $file) {
			...
			try {
				...
				$i++;		
					...
					$page->ajaxReplace[] = array('CompressRespond', 'arg', $e);
					return ;
				}
			clearstatcache($file);	
		$data=$this->Get_Cache();
		...
		$page->ajaxReplace[] = array('CompressRespond', 'arg', $arg_value);
		return ;
	}
	...
	function Get_Images($dir_rel = ''){
		...
		if($this->opts['thumb']<>"") {$temp=".";} else {$temp='thumbnails';}
	...
	function human_filesize($bytes, $decimals = 2) {
	 ...
	 function IsImgValid($file){
			$img_types = array('png'=>1,'jpg'=>1,'jpeg'=>1);
